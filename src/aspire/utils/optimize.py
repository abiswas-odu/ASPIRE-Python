...
def conj_grad(a_fun, b, cg_opt=None, init=None):
    ...
    r_cu = xp.asarray(r)
    s_cu = cg_opt['preconditioner'](r_cu.copy())
    ...
    if xp.any(x != 0):
        ...
        a_x = a_fun(x)
        r_cu = r_cu-a_x
        s_cu = cg_opt['preconditioner'](r_cu)
    else:
    ...
    if b_norm == 0:
        ...
        return xp.asnumpy(x), xp.asnumpy(obj), info
    ...
    for i in range(1, cg_opt['max_iter'] + 1):
        ...
        a_p = a_fun(p_cu)
        old_gamma = xp.real(xp.sum(s_cu.conj() * r_cu, -1))

        alpha = old_gamma / xp.real(xp.sum(p_cu.conj() * a_p, -1))
        x += xp.expand_dims(alpha, axis=1) * p_cu
        a_x += xp.expand_dims(alpha, axis=1) * a_p

        r_cu -= xp.expand_dims(alpha, axis=1) * a_p
        s_cu = cg_opt['preconditioner'](r_cu.copy())
        #s_cu = r_cu.copy()

        new_gamma = xp.real(xp.sum(r_cu.conj() * s_cu, -1))
        beta = new_gamma / old_gamma
        p_cu *= xp.expand_dims(beta, axis=1)
        p_cu += s_cu

        obj = (xp.real(xp.sum(x.conj() * a_x, -1)
                - 2 * xp.real(xp.sum(xp.conj(b * x), -1))))
        res = xp.sqrt(xp.sum(r_cu ** 2, -1))

        #info['iter'].append(i)
        #info['res'].append(res)
        #info['obj'].append(obj)
        #if cg_opt['store_iterates']:
        #    info['x'].append(xp.asnumpy(x))
        #    info['r'].append(xp.asnumpy(r_cu))
        #    info['p'].append(p)
